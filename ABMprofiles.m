...
% This is for France
%feat = [ 0.45, 2, 0.925,0, 0.015, 0.164, 0.588, 0.233, 0, 2];
% This is for Libya
...
% This is for the profiles at different time points
% The first spot is the run, 1st run, 2nd run...
% 2nd spot is the iteration step
% The last 13 are their profiles
Tprof = zeros(400*ttrun, 15);


wL = ww(j,1).*ww(j,2:8);
wF = ww(j,9).*ww(j,10:16);
wP = ww(j,17).*ww(j,18:23);
wT = ww(j,25:27);
events =zeros(ttrun*20, 1);
%
for k = 1:ttrun
...
s = RandStream('mt19937ar','Seed',20, 'NormalTransform', 'Ziggurat');
        ...
        %FT to C
         if(cha(zs,1) == 0 )
           ...
           if(cha(zi, 9) > powL2 && cha(zs,10) < .2)
                ...
                z2 = find(cha(:,1) >1);
                l2 = length(z2);
                Tprof(spo:(spo+l2-1), 1) = k;
                Tprof(spo:(spo+l2-1), 2) = jj;
                Tprof(spo:(spo+l2-1), 3:end) = cha(z2,:);
                spo = spo+l2;
                ...
                % increase exposure of violence
                 [ ~, b] = find(A(z2(1:y),:) > 0);
                    ...
                    b = intersect(b, find(cha(:,1) ==0));
                            if(length(A) ~= n)
                           msg = 'Error occurred!!! Annoying 911';
                             error(msg)
                            end
